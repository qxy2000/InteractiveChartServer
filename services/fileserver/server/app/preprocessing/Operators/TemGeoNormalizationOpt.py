    ...
    def run(self):
        for field in list(self.data.data['field'].values()):
            ...
            if field['type']=='temporal':
                ...
                self.data.set_col_label(field['field'],"values",list(map(lambda x: str(x),new_df.dropna().unique().tolist()))  )
...
def _ymdformat(column,year=1900,month=1,day=1):
    ...
    try:
        ...
        try:        
            ...
            try:
                b = datetime.datetime.strptime(str(column),'%Y%m%d').strftime('%Y/%m/%d')
            except: 
                b = column
            ...
            if ":" in df[1]:
                try:
                    ...
                    try:
                        ...
                        try:
                            df=pd.to_datetime(df,format='%Y-%m-%d %H:%M:%S').dt.strftime('%Y/%m/%d')
                        except:
