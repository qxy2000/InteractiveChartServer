...
class LabelTypeOpt(Operator):
    ...
    def start(self):
        self.glove_6b50d = cPickle.load(open("glove_6b50d.pkl","rb"))
        f = open(r"list_fasttext.txt")
        line = f.readline()
        self.dataset_EN = []
        while line:
            self.dataset_EN.append(line.split('\n')[0])
            line = f.readline()
        f.close()
    ...
    def run(self):
        ...
        self.df_type = type_checker(df,self.keywords,self.geodict)
        self.df_type['pictype']=get_similar_glove(self.colname,self.glove_6b50d,self.dataset_EN)
    ...
    def finish(self):
        if self.df_type['type']=='temporal':
            ...
            self.data.set_col_label(self.colname,"pictype",self.df_type['pictype'])
...
def type_checker(df,keywords,geodict):
    ...
    if any(i in df.name.lower() for i in ['description']):
        ...
        df_type["type"] = "text"
    elif any(i in df.name.lower() for i in keywords['temporal']): 
        df_type=_temporalcheck(df,df_type)
    elif any(i in df.name.lower() for i in keywords['categorical']) or df.name.lower()=='id' or 'ID' in df.name :  
        df_type=_categoricalcheck(df,df_type)
    elif any(i in df.name.lower() for i in keywords['geographical']):
        df_type=_geographiccheck(df,df_type,geodict)
    else:
        df_type= _dtypecheck(df,df_type,geodict,keywords)
    ...
    if 'year' in df.name.lower() or '年' in df.name :
        if df.dtype =='int64':
            if any(df<1) or any(df>2050):
                ...
                df_type["measure"]=['sum','min','max','avg']
                df_type["mean"] = df.mean()
                df_type["median"] = df.median()
                df_type["std"] = df.std()
                df_type["var"] = df.var()
                df_type["values"] = list(map(lambda x: str(x),df.dropna().unique().tolist()))
            else:
...
def _geographiccheck(df,df_type,dictory):
    if df.name.strip().lower().startswith('lat') or df.name.strip().lower().startswith('long') or df.name.strip().lower().startswith('经度') or df.name.strip().lower().startswith('纬度'):
    ...
    elif any(i in df.name.lower() for i in ['zipcode','postcode','zip code','post code','areacode', 'area code','邮编']):
        df_type["type"] = "geographical"
        df_type["values"] = df.fillna('unknown').unique().tolist()
        ...
        if df.dtype =='int64':
            ...
            df_type['subtype']='world'
    elif 'state' in df.name.lower() or 'location' in df.name.lower() or '省' in df.name.lower() or 'province' in df.name.lower() or '州' in df.name.lower():
        df_type["type"] = "geographical"
        df_type["values"] = df.fillna('unknown').unique().tolist()
        df_type['isPostCode']=False
        try:
        ...
        if type(df.dropna().any()) is str and (any(df[1] in x for x in dictory['china_city'].keys()) or any(df[1] in x for x in dictory['china_en'].values())):
            df_type['subtype']='china'
        elif type(df.dropna().any()) is str and any(df[1].strip() in x for x in dictory['US_city'].keys()):
            df_type['subtype']='usa'
        elif type(df.dropna().any()) is str and any(df[1].strip() in x for x in dictory['abbrev_us_state'].keys()):
            df_type['subtype']='usa'        
        ...
        if type(df.dropna().any()) is str and any(df[1] in x for x in numpy.concatenate(list(dictory['china_city'].values()))) :
            df_type['subtype']='china'
        elif type(df.dropna().any()) is str and any(df[1] in x for x in numpy.concatenate(list(dictory['US_city'].values()))):
            df_type['subtype']='usa'
        elif type(df.dropna().any()) is str:
            df_type['subtype']='world'   
        else:
...
def _countries(name,dictory):
    _countries=dictory['countries']
    result = False
    name = name.lower()
...
def _dtypecheck(df,df_type,dictory,keywords):
    #通过field的dtype判断数据种类
    if df.dtype =='int64' and all(df[i]<df[i+1] for i in range(20)) and all(df<=df.shape[0]):
        ...
        df_type["measure"]=['sum','min','max','avg']
        if any(i in df.name.lower() for i in keywords['aggregation']['no_sum_avg']):
            df_type["measure"]=['min','max']
        elif any(i in df.name.lower() for i in keywords['aggregation']['no_sum']):
            df_type["measure"]=['avg','min','max']
        elif any(i in df.name.lower() for i in keywords['aggregation']['only_avg']):
            df_type["measure"]=['avg']
        df_type["type"] = "numerical"
        df_type["mean"] = df.mean()
        df_type["median"] = df.median()
